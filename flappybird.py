...
game_images = {}
framepersecond = 32
pipeimage = 'images/Icons_City.png'
background_image = 'images/background.jpg'
birdplayer_image = 'images/car.png'
...
def flappygame():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.key == K_UP and vertical>0:
          
                # decrement in y co-ordinate
                vertical -= vel
          
            # if left arrow key is pressed   
            if event.key == K_DOWN and vertical<500-height:
...
def isGameOver(horizontal, vertical, up_pipes, down_pipes):
    if vertical > elevation - 93 or vertical < 0:
        return True
  
    for pipe in up_pipes:
        pipeHeight = 55
        if(vertical < pipeHeight + pipe['y'] and\
           abs(horizontal - pipe['x']) < 52):
            return True
  
    for pipe in down_pipes:
        if (vertical + 50 > pipe['y']) and\
        abs(horizontal - pipe['x']) < 100:
            return True
    return False
...
def createPipe():
    ...
    y2 = offset + \
        random.randrange(
            0, int(window_height - 1.2 * offset))  
